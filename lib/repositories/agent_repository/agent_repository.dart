...
class AgentRepository {
  ...
  Future<Map<String, dynamic>> getAgentRequests(var agentId) async {
    print('ccc');
    dynamic response =
        await _apiService.getApi('${AppUrl.getAllRequestsofAgent}/$agentId');
    return response;
  }

  Future<Map<String, dynamic>> sendPaymentRequest(var data) async {
    dynamic response =
        await _apiService.postApi(data, AppUrl.sendPaymentRequesttoUser);
    return response;
  }

  Future<Map<String, dynamic>> getAllPropertiesInfoOfAgent(var data) async {
    dynamic response = await _apiService
        .getApi('${AppUrl.getAllPropertiesInfoOfAgent}/${data}');
    return response;
  }
}
